             ...
             NMSettingSecretFlags pw_flags)
{
	...
	if (   !(pw_flags & NM_SETTING_SECRET_FLAG_NOT_SAVED)
		...
		if (in_pw)
			...
			pw = keyring_lookup_secret (vpn_uuid, NM_SSTP_KEY_PASSWORD);
	}
	...
	if (external_ui_mode) {
		...
		keyfile_add_entry_info (keyfile, NM_SSTP_KEY_PASSWORD, pw ? pw : "", _("Password:"), TRUE, allow_interaction);
...
main (int argc, char *argv[])
{
	...
	nm_vpn_service_plugin_get_secret_flags (secrets, NM_SSTP_KEY_PASSWORD, &pw_flags);

	if (!get_secrets (vpn_uuid, vpn_name, retry, allow_interaction, external_ui_mode,
	                  g_hash_table_lookup (secrets, NM_SSTP_KEY_PASSWORD),
	                  &password,
	                  pw_flags))
		return 1;
